...
class ControlCenter(QWidget):
    ...
    def submit_clicked(self):
        ...
        # if current tab is ROS_1
            ...
            ros_mannual = "1.getStatus\n2.tts <string>\n3.moveArm <'l'|'r'> <float> <float> <float> <float>\n4.standby\n5.simpleWalk <int> N\n6.moveHead <float> <float> <float>\n7.stand\nSpace between two argument is MUST!\n"
            self.ros_1_str += '>> '
            ...
            # if yes, return busy message
            ...
            elif 'simpleWalk' in instruction:
                try:
                    if not os.path.exists('robot_control/output'):
                        file = open("robot_control/output","w") 
                        file.write(instruction) 
            ...
            elif 'tts' in instruction:
                try:
                    if not os.path.exists('robot_control/output'):
                        file = open("robot_control/output","w") 
                        file.write(instruction) 
                        file.close() 
                        self.ros_1_str += self.ros.tts();
                    else:
                        self.ros_1_str += "Other command is in process\n"
                except FileNotFoundError:
                    self.ros_1_str += "Directory: robot_control not detected\n"
                ...
                self.ros_1_str += "No command matches, please check the format of command by entering 'help'\n"
